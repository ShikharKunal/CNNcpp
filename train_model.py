...
def load_config(config_path):
    ...
    config.setdefault('weights_dir', 'weights')
    config.setdefault('plots_dir', 'plots')
    config.setdefault('save_metrics', True)
    config.setdefault('generate_plots', True)
    config.setdefault('print_batch_interval', 200)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train CNN with mydl using configuration file",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example usage:
  python3 train_model.py --config config/default_train.yaml --dataset data_1_split/train
        """
    )
    parser.add_argument("--config", type=str, required=True, help="Path to training configuration file (YAML/JSON)")
    ...
    plots_dir = script_dir / config['plots_dir'] / timestamp
    ...
    weights_path = script_dir / config['weights_dir'] / f"{timestamp}.bin"
    ...
    print(f"Batch Log:  Every {config['print_batch_interval']} batches")
    print("=" * 50)
    print()
    ...
    loader = DataLoader(images, labels, num_classes, config['batch_size'])
    ...
    for epoch in range(config['epochs']):
        ...
        print(f"Starting epoch {epoch + 1}/{config['epochs']}...", flush=True)
        ...
        while loader_has_next():
            ...
            if n_batches % config['print_batch_interval'] == 0:
        ...
        print(f"Epoch {epoch + 1}/{config['epochs']}  Loss: {avg_loss:.4f}  Accuracy: {acc:.4f}  Time: {epoch_time:.2f}s", flush=True)
